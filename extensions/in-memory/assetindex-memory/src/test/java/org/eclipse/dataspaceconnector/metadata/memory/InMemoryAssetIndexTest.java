...
class InMemoryAssetIndexTest {
    ...
    private InMemoryAssetStorage repository;

    @BeforeEach
    void setUp() {
        repository = new InMemoryAssetStorage();
        index = new InMemoryAssetIndex(repository, new CriterionToPredicateConverter());
    }
    ...
    void queryAssets_multipleFound() {
        ...
        repository.add(testAsset1);
        repository.add(testAsset2);
        repository.add(testAsset3);
    ...
    void queryAllAssetsWithCursor() {
        ...
        Cursor cursor = null;
        do {
            var query = AssetIndexQuery.Builder.newInstance().expression(expression).limit(3).cursor(cursor).build();
            var resultSet1 = index.queryAssets(query);
            for (var ignored : resultSet1) {
                assetsQueried += 1;
            }
            cursor = resultSet1.getCursor();
