...
public class MultipartController {
    ...
    public MultipartController(
            final MultipartRequestHandlerResolver multipartRequestHandlerResolver,
    ...
    public Response request(
            @FormDataParam("header") final RequestMessage header,
            @FormDataParam("payload") final String payload) {
        if (header == null) {
            return Response.status(Response.Status.BAD_REQUEST).build();
        }

        final MultipartRequest multipartRequest = MultipartRequest.Builder.newInstance()
                .header(header)
                .payload(payload)
                .build();

        final MultipartRequestHandler multipartRequestHandler = multipartRequestHandlerResolver
                .resolveHandler(multipartRequest)
                .orElse(rejectionMultipartRequestHandler);

        final MultipartResponse multipartResponse = multipartRequestHandler.handleRequest(multipartRequest);
