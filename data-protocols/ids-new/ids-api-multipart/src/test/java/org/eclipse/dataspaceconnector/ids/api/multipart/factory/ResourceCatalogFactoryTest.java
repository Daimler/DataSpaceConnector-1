...
public class ResourceCatalogFactoryTest {
    ...
    void addsResourcesToCatalog() {
        final Resource expected = EasyMock.mock(Resource.class);
        final List<Resource> expectedResources = new ArrayList<>(Collections.singletonList(expected));

        final ResourceCatalog catalog = resourceCatalogFactory.createResourceCatalogBuilder(expectedResources);
        final ArrayList<? extends Resource> resources = catalog.getOfferedResource();
