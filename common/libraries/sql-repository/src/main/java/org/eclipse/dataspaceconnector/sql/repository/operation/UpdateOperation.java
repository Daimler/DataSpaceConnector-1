...
public class UpdateOperation {
    ...
    public UpdateOperation(@NotNull SqlClient sqlClient) {
    ...
    public void invoke(@NotNull Asset asset) throws SQLException {
        ...
        List<Boolean> existsResult = sqlClient.execute(new ExistsMapper(), sqlAssetExists, asset.getId());
        ...
        try {
            ...
            List<Property> storedProperties = sqlClient.execute(new PropertyMapper(), sqlProperties, asset.getId());
            ...
            for (var property : propertyToDelete) {
                sqlClient.execute(sqlPropertyDelete, asset.getId(), property.getKey());
            }
            for (var property : propertyToInsert) {
                String value = EnvelopePacker.pack(property.getValue());
                sqlClient.execute(sqlPropertyCreate, asset.getId(), property.getKey(), value);
            }
            for (var property : propertyToUpdate) {
                String value = EnvelopePacker.pack(property.getValue());
                sqlClient.execute(sqlPropertyUpdate, value, asset.getId(), property.getKey());
            }
